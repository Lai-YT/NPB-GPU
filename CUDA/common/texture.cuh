...
template <typename T>
cudaError_t malloc3DTextureObject(TextureObject<T> *texObj, size_t width,
  ...
  err = cudaMalloc3DArray(&texObj->devPtr, &texObj->fmtDesc, extent);
  if (err != cudaSuccess) {
    return err;
  }
  ...
  texObj->texDesc.addressMode[0] = cudaAddressModeWrap;
  texObj->texDesc.addressMode[1] = cudaAddressModeWrap;
  texObj->texDesc.addressMode[2] = cudaAddressModeWrap;
  texObj->texDesc.readMode = cudaReadModeElementType;
  texObj->texDesc.normalizedCoords = 0;
...
template <typename T>
cudaError_t memcpy3DTextureObject(TextureObject<T> *texObj, T *data, size_t width,
                                  size_t height, size_t depth) {
  cudaMemcpy3DParms copyParams = {0};
  copyParams.srcPtr = make_cudaPitchedPtr(
      (void *)data, width * sizeof(T), width, height);
  copyParams.dstArray = texObj->devPtr;
...
template <typename T>
cudaError_t malloc3DSurfaceObject(SurfaceObject<T> *surfObj, size_t width,
                                  size_t height, size_t depth) {
  cudaError_t err;
  ...
  surfObj->fmtDesc = cudaCreateChannelDesc<T>();
  cudaExtent extent = make_cudaExtent(width, height, depth);
  err = cudaMalloc3DArray(&surfObj->devPtr, &surfObj->fmtDesc, extent,
                          cudaArraySurfaceLoadStore);
  if (err != cudaSuccess) {
    return err;
  }
...
template <typename T>
cudaError_t memcpy3DToSurfaceObject(SurfaceObject<T> *surfObj, const T *data,
                                    size_t width, size_t height, size_t depth) {
  cudaMemcpy3DParms copyParams = {0};
  copyParams.srcPtr = make_cudaPitchedPtr(
      (void *)data, width * sizeof(T), width, height);
  copyParams.dstArray = surfObj->devPtr;
  copyParams.extent = make_cudaExtent(width, height, depth);
  copyParams.kind = cudaMemcpyHostToDevice;
  return cudaMemcpy3D(&copyParams);
}
...
template <typename T>
cudaError_t memcpy3DFromSurfaceObject(T *dst, SurfaceObject<T> *surfObj, size_t width,
                                      size_t height, size_t depth) {
  cudaMemcpy3DParms copyParams = {0};
  copyParams.srcArray = surfObj->devPtr;
  copyParams.dstPtr = make_cudaPitchedPtr(
      (void *)dst, width * sizeof(T), width, height);
  copyParams.extent = make_cudaExtent(width, height, depth);
