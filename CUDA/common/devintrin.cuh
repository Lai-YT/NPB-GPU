...
__device__ __forceinline__ T tex1Ds32(cudaTextureObject_t tex, const int x) {
  static_assert(::cuda::std::is_same<T, float>::value,
...
template <typename T = float>
__device__ __forceinline__ T tex3Ds32(cudaTextureObject_t tex, const int x,
                                      const int y, const int z) {
  static_assert(cuda::std::is_same<T, float>::value, "Only support float type");
  T a, b, c, d;
  /* %8 is required to pad four-element tuple and is ignored */
  asm volatile("tex.3d.v4.f32.s32 {%0, %1, %2, %3}, [%4, {%5, %6, %7, %8}];\n\t"
               : "=f"(a), "=f"(b), "=f"(c), "=f"(d)
               : "l"(tex), "r"(x), "r"(y), "r"(z), "r"(0));
  return a;
}
